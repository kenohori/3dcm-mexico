...
- **`elevadormx` assumes pre-generated inputs.** It reads building footprints, water bodies, plant cover, and terrain from vector layers (`.gpkg`) — water bodies, plant cover, and terrain are still done in Python/QGIS. Building footprints and road polygons, however, can now be generated in-tool: roads from the INEGI `manzana_a` city-block layer, optionally subtracting water bodies (`--waterbody`) and land-use layers (`--land_use`) via GEOS Boolean operations (`--generate_roads true`, skipping the `--road` input); the generated gaps are then classified as Road/Railway/WaterBody by proximity to INEGI line layers (`--road_lines`/`vialidad_l`, `--railway_lines`/`via_ferrea_l`, `--stream_lines`/`corriente_ag_l`; ties go to Road, and gaps beyond `line_classification_distance` stay Road). The line layers are noded into segments with `UnaryUnion` (splitting lines at their mutual intersections), each road triangle is classified by its nearest segment, and adjacent triangles sharing a segment are merged into polygons carrying that segment's class and attributes in the CityJSON object; building footprints by raster→polygon of the region-growing labels (`--buildings_output`), simplified in-tool with Visvalingam–Whyatt (`--simplify_tolerance`, default 3 m). When both `--grow_output` and `--buildings_output` are set, the generated footprints are loaded into the model automatically in the same run and any `--building` input is ignored (a warning is printed if both are given).
