...
- It reads every `*_ms.hdr` under `--dsm-dir`, clusters the tiles into the sheet's row/column grid, and places each interior seam at the midpoint of the overlap band shared by every tile pair in that gap. Interior cells use the seam coordinates; outer cells use each tile's own extent. The generated cells tile the sheet exactly.
- It writes one config JSON per tile under `--out-dir/configs/` (final OBJ/CityJSON/terrain.obj into `--out-dir/<tile>/`, intermediate GPKGs/TIFs into `--out-dir/work/<tile>/`) and runs `elevadormx` per tile, with `--jobs` parallelism.
- Run it as `python3 scripts/run_tiles.py --jobs 4` (adjust `--binary` to the Release binary, `--dsm-dir`/`--dtm-dir`/topo layer paths to taste). `--generate-configs-only` emits the configs without running; `--tile <id>` restricts to one tile.
- The whole-tile outputs of a previous run (`data/output/tiles/`) are re-runnable; intermediates live in `data/output/tiles/work/` and are safe to delete to reclaim ~3.7 GB.

## Downloading more test data

`scripts/download_neighbours.py` fetches INEGI data for the eight 1:50k sheets surrounding E14A39 (Cuautitlán, Villa del Carbón, Toluca de Lerdo, Tenango de Arista, Milpa Alta, Texcoco, Chalco, Amecameca):

- the topographic vector product (newest edition), extracted to `data/topo/<upc>_s/`; and
- every subtile that has a 1.5 m DSM/DTM product (the same BIL product E14A39 uses), installed as `data/dsm/<tile>/<tile>_ms.bil` and `data/dtm/<tile>/<tile>_mt.bil` — the layout `run_tiles.py` scans.

Run it as `python3 scripts/download_neighbours.py --jobs 4` (full fetch ≈ 9 GB of zips, cached under `data/downloads/`, safe to delete after install). Re-runs skip anything already downloaded or installed, so interrupting is safe. `--dry-run` prints the plan; `--sheets`/`--tiles` restrict it; `--skip-topo`/`--skip-elev` pick a side. The embedded UPCs were gathered from the INEGI catalog on 2026-08-21; subtiles without a 1.5 m product (only 5 m or nothing) are skipped and listed. The script warns per sheet when the topo edition lacks `manzana_a` (see the schema gotcha below).
...
- **`elevadormx` assumes pre-generated inputs.** It reads building footprints, water bodies, plant cover, and terrain from vector layers (`.gpkg`) — all of these, however, can now be generated in-tool: roads from the INEGI `manzana_a` city-block layer, optionally subtracting water bodies (`--waterbody`, or the `--water_areas` layers when `--generate_waterbodies`) and land-use layers (`--land_use`) via GEOS Boolean operations (`--generate_roads true`, skipping the `--road` input); the road area is then classified as Road/Railway/WaterBody against the INEGI line layers (`--road_lines`/`vialidad_l`, `--railway_lines`/`via_ferrea_l`, `--stream_lines`/`corriente_ag_l`): the lines are noded into segments with `UnaryUnion` (splitting them at their mutual intersections), each road triangle is classified by its nearest segment (ties go to Road, and triangles beyond `line_classification_distance` stay Road), and adjacent triangles sharing a segment are merged into polygons carrying that segment's class and attributes in the CityJSON object; building footprints by raster→polygon of the region-growing labels (`--buildings_output`), simplified in-tool with Visvalingam–Whyatt (`--simplify_tolerance`, default 3 m). When both `--grow_output` and `--buildings_output` are set, the generated footprints are loaded into the model automatically in the same run and any `--building` input is ignored (a warning is printed if both are given); the loaded footprints are **clipped to the study area** via `read_clipped_polygon_features` (not `read_polygon_layer`), so a run bounded by `study_area` never emits geometry outside it — required for the non-overlapping tile outputs of `scripts/run_tiles.py`. Region growing itself still runs over the whole tile raster, so footprints outside the study area are found and then discarded; plant cover from the INEGI `area_publica_a` public-areas layer, clipped to the study area via GEOS `Intersection` (`--generate_plantcover true` + `--public_areas`, skipping the `--plantcover` input; a warning is printed if both are given, and the generated polygons can be written with `--plantcover_output`); water bodies from the INEGI areal water layers `--water_areas` (`cuerpo_agua_a`, `estanque_a`, `canal_a`, `corriente_ag_a`), each clipped to the study area via GEOS `Intersection` (`--generate_waterbodies true`, skipping the `--waterbody` input; a warning is printed if both are given, and the generated polygons can be written with `--waterbody_output`); terrain from the INEGI `manzana_a` city-block layer, unioned and clipped to the study area via GEOS `UnionCascaded`/`Intersection` (`--generate_terrain true`, reusing the `--city_blocks` input; skipping the `--terrain` input, with a warning if both are given, and the generated polygons can be written with `--terrain_output`).
- **Topo vector editions use two different schemas.** The 2021+ "Conjunto de Datos Vectoriales" editions (E14A39, E14A29, E14A49 2021; E14B31 2023; E14A28 2020) have `manzana_a`, `area_publica_a`, `vialidad_l` and (except E14A28) `via_ferrea_l` — the layers the in-tool generators need. The 2019 editions (E14A38, E14A48, E14B21, E14B41) predate that schema: names carry a `50` infix (`cuerpo_agua50_a`, `carretera50_l`) and `manzana_a`/`area_publica_a`/`vialidad_l` do not exist, so `generate_terrain`/`generate_roads`/`generate_plantcover` cannot run on them (a missing city-blocks file makes `generate_road_polygons` return early, skipping the water/land-use layers after it). Some newer-schema editions also omit layers a sheet simply lacks (no `corriente_ag_a` in E14A29/E14A49/E14B31, no `canal_a`/`via_ferrea_l` in E14A28): a missing `--water_areas` member prints an error and is skipped, but per-sheet layer paths still need to point at files that exist.
- **Generated PlantCover, WaterBody and Terrain polygons overlap in the source data and are resolved in-tool.** The INEGI area layers (`area_publica_a`, `cuerpo_agua_a`/`estanque_a`/`canal_a`/`corriente_ag_a`, `manzana_a`) overlap each other, so after all layers are loaded `resolve_area_overlaps` carves the shared areas out with a fixed priority **WaterBody > PlantCover > Terrain** (the higher-priority class keeps the shared area; GEOS `UnionCascaded`/`Difference`/`Union` handle the shared/partially-collinear boundaries robustly). It runs unconditionally on the three classes regardless of whether they were generated in-tool or read from input files; polygons are split (ids get a `-N` suffix) and fully-covered ones are dropped. The `.gpkg` outputs written by the generators (`--plantcover_output` etc.) still contain the raw, un-resolved polygons.
